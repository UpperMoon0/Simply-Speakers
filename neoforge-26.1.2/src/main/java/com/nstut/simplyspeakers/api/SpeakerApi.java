...
public final class SpeakerApi {
    ...
    private static void broadcastStateChange(Level level, BlockPos pos, SpeakerState state) {
        ...
        dev.architectury.networking.NetworkManager.sendToPlayers(serverLevel.players(), packet);
        ...
        if (!state.isPlaying() || state.isPaused()) {
            ...
            for (net.minecraft.server.level.ServerPlayer player : serverLevel.players()) {
                dev.architectury.networking.NetworkManager.sendToPlayer(player, stopPacket);
            }
        }
    }
