...
public class PlayAudioPacketS2C {
    ...
    public PlayAudioPacketS2C(BlockPos pos, String audioId, float playbackPositionSeconds, boolean isLooping) {
        this.pos = pos;
    ...
    public static void handle(PlayAudioPacketS2C pkt, Supplier<NetworkManager.PacketContext> ctxSupplier) {
        ...
        if (Minecraft.getInstance().level.isClientSide) {
            context.queue(() -> {
                ...
                AudioFileMetadata metadata = new AudioFileMetadata(pkt.audioId, ""); // Filename not needed for playback
