package com.nstut.simplyspeakers.network;

import dev.architectury.networking.NetworkChannel;
import net.minecraft.resources.ResourceLocation;
import com.nstut.simplyspeakers.SimplySpeakers;

public class PacketRegistries {
    private static NetworkChannel channel;
    private static boolean registered = false;
    
    public static NetworkChannel getChannel() {
        if (channel == null) {
            channel = NetworkChannel.create(
                ResourceLocation.fromNamespaceAndPath(SimplySpeakers.MOD_ID, "main")
            );
        }
        return channel;
    }

    public static void registerC2S() {
        if (registered) return;
        NetworkChannel ch = getChannel();
        // Client to Server packets
        ch.register(LoadAudioCallPacketC2S.class,
                LoadAudioCallPacketC2S::encode,
                LoadAudioCallPacketC2S::new,
                LoadAudioCallPacketC2S::handle
        );
        ch.register(AudioPathPacketC2S.class,
                AudioPathPacketC2S::encode,
                AudioPathPacketC2S::new,
                AudioPathPacketC2S::handle
        );
        ch.register(ToggleLoopPacketC2S.class,
                ToggleLoopPacketC2S::encode,
                ToggleLoopPacketC2S::new,
                ToggleLoopPacketC2S::handle
        );
        ch.register(RequestUploadAudioPacketC2S.class,
                RequestUploadAudioPacketC2S::encode,
                RequestUploadAudioPacketC2S::new,
                RequestUploadAudioPacketC2S::handle
        );
        ch.register(UploadAudioDataPacketC2S.class,
                UploadAudioDataPacketC2S::encode,
                UploadAudioDataPacketC2S::new,
                UploadAudioDataPacketC2S::handle
        );
        ch.register(RequestAudioListPacketC2S.class,
                RequestAudioListPacketC2S::encode,
                RequestAudioListPacketC2S::new,
                RequestAudioListPacketC2S::handle
        );
        ch.register(SelectAudioPacketC2S.class,
                SelectAudioPacketC2S::encode,
                SelectAudioPacketC2S::new,
                SelectAudioPacketC2S::handle
        );
        ch.register(RequestAudioFilePacketC2S.class,
                RequestAudioFilePacketC2S::encode,
                RequestAudioFilePacketC2S::new,
                RequestAudioFilePacketC2S::handle
        );
        ch.register(StopPlaybackPacketC2S.class,
                StopPlaybackPacketC2S::encode,
                StopPlaybackPacketC2S::new,
                StopPlaybackPacketC2S::handle
        );
        ch.register(SetSpeakerIdPacketC2S.class,
                SetSpeakerIdPacketC2S::encode,
                SetSpeakerIdPacketC2S::new,
                SetSpeakerIdPacketC2S::handle
        );
        ch.register(UpdateMaxVolumePacketC2S.class,
                UpdateMaxVolumePacketC2S::encode,
                UpdateMaxVolumePacketC2S::new,
                UpdateMaxVolumePacketC2S::handle
        );
        ch.register(UpdateMaxRangePacketC2S.class,
                UpdateMaxRangePacketC2S::encode,
                UpdateMaxRangePacketC2S::new,
                UpdateMaxRangePacketC2S::handle
        );
        ch.register(UpdateAudioDropoffPacketC2S.class,
                UpdateAudioDropoffPacketC2S::encode,
                UpdateAudioDropoffPacketC2S::new,
                UpdateAudioDropoffPacketC2S::handle
        );
        ch.register(UpdateProxyMaxVolumePacketC2S.class,
                UpdateProxyMaxVolumePacketC2S::encode,
                UpdateProxyMaxVolumePacketC2S::new,
                UpdateProxyMaxVolumePacketC2S::handle
        );
        ch.register(UpdateProxyMaxRangePacketC2S.class,
                UpdateProxyMaxRangePacketC2S::encode,
                UpdateProxyMaxRangePacketC2S::new,
                UpdateProxyMaxRangePacketC2S::handle
        );
        ch.register(UpdateProxyAudioDropoffPacketC2S.class,
                UpdateProxyAudioDropoffPacketC2S::encode,
                UpdateProxyAudioDropoffPacketC2S::new,
                UpdateProxyAudioDropoffPacketC2S::handle
        );
    }

    public static void registerS2C() {
        NetworkChannel ch = getChannel();
        // Server to Client packets
        ch.register(StopAudioPacketS2C.class,
                StopAudioPacketS2C::encode,
                StopAudioPacketS2C::new,
                StopAudioPacketS2C::handle
        );
        ch.register(PlayAudioPacketS2C.class,
                PlayAudioPacketS2C::encode,
                PlayAudioPacketS2C::new,
                PlayAudioPacketS2C::handle
        );
        ch.register(SpeakerBlockEntityPacketS2C.class,
                SpeakerBlockEntityPacketS2C::encode,
                SpeakerBlockEntityPacketS2C::new,
                SpeakerBlockEntityPacketS2C::handle
        );
        ch.register(RespondUploadAudioPacketS2C.class,
                RespondUploadAudioPacketS2C::encode,
                RespondUploadAudioPacketS2C::new,
                RespondUploadAudioPacketS2C::handle
        );
        ch.register(AcknowledgeUploadPacketS2C.class,
                AcknowledgeUploadPacketS2C::encode,
                AcknowledgeUploadPacketS2C::new,
                AcknowledgeUploadPacketS2C::handle
        );
        ch.register(SendAudioListPacketS2C.class,
                SendAudioListPacketS2C::encode,
                SendAudioListPacketS2C::new,
                SendAudioListPacketS2C::handle
        );
        ch.register(SendAudioFilePacketS2C.class,
                SendAudioFilePacketS2C::encode,
                SendAudioFilePacketS2C::new,
                SendAudioFilePacketS2C::handle
        );
        ch.register(SpeakerStateUpdatePacketS2C.class,
                SpeakerStateUpdatePacketS2C::encode,
                SpeakerStateUpdatePacketS2C::new,
                SpeakerStateUpdatePacketS2C::handle
        );
    }
    
    public static void init() {
        if (registered) return;
        registered = true;
        registerC2S();
        registerS2C();
    }

    // Sending packets:
    // To server: NetworkManager.sendToServer(getChannel(), new MyPacket(...));
    // To client(s): NetworkManager.sendToPlayers(listOfPlayers, getChannel(), new MyPacket(...));
    // Or: NetworkManager.sendToPlayer(player, getChannel(), new MyPacket(...));
    // Or: NetworkManager.sendToClients(serverLevel, getChannel(), new MyPacket(...));
}
